...
# 本地目录部署脚本 - 专门用于在项目目录内运行
# 使用方法: sudo ./deploy-local.sh

set -e

# 配置变量
PROJECT_NAME="translation-service"
PROJECT_DIR=$(pwd)  # 使用当前目录作为项目目录
...
case "$1" in
    start) sudo systemctl start $SERVICE_NAME; echo "服务已启动" ;;
    stop) sudo systemctl stop $SERVICE_NAME; echo "服务已停止" ;;
    restart) sudo systemctl restart $SERVICE_NAME; echo "服务已重启" ;;
    ...
    test)
        ...
        echo ""
        ;;
    *) echo "使用方法: $0 {start|stop|restart|status|logs|test}"; exit 1 ;;
esac
EOF
