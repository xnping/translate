...
class TranslationService:
    ...
    def __init__(self):
        self.app_id = config.get('baidu_api', 'app_id')
        self.secret_key = config.get('baidu_api', 'secret_key')
        self.base_url = config.get('baidu_api', 'base_url')
        self.timeout = config.get('baidu_api', 'timeout')
    ...
    async def translate_single(self, text: str, from_lang: str = "auto", to_lang: str = "zh", 
        ...
        # 检查缓存
        if use_cache:
            cache_key = cache_manager.generate_cache_key(from_lang, to_lang, text)
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        ...
                        if use_cache:
                            ...
                            await cache_manager.set(cache_key, cache_result)
    ...
    async def translate_batch(self, items: List[Dict[str, Any]], from_lang: str = "auto", 
        ...
        if use_cache:
            ...
            # 批量查询缓存
            if cache_keys:
                cached_results = await cache_manager.batch_get(list(cache_keys.keys()))
        ...
        if cache_updates and use_cache:
            await cache_manager.batch_set(cache_updates)
